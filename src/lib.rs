...
use bincode::Options;
// base64 is no longer needed since we pass binary data directly
...
impl Guest for Filecontext {
  ...
  fn loadremotehierarchy(hierarchy_data: Vec<u8>) {
  ...
  fn loadremotetimetable(timetable_data: Vec<u8>) {
  ...
  fn loadremotesignals(signals_data: Vec<u8>) {
  ...
  fn loadremotehierarchychunk(chunk_data: Vec<u8>, chunk_index: u32, total_chunks: u32) {
    outputlog(&format!("Received hierarchy chunk {}/{} ({} bytes)", chunk_index + 1, total_chunks, chunk_data.len()));
    
    let mut chunks = _hierarchy_chunks.lock().unwrap();
    let mut total_chunks_ref = _hierarchy_total_chunks.lock().unwrap();
    ...
    if all_received {
      outputlog("All hierarchy chunks received, reassembling...");
      
      // Reassemble all chunks into one Vec<u8>
      let mut reassembled_data = Vec::new();
      for chunk in chunks.iter() {
        reassembled_data.extend_from_slice(&chunk);
      }
      
      outputlog(&format!("Reassembled hierarchy data: {} bytes", reassembled_data.len()));
      
      // Process the reassembled data using the original function logic
      Self::loadremotehierarchy(reassembled_data);
      
      // Clear chunks for next use
      chunks.clear();
      *total_chunks_ref = 0;
    }
  }

  fn loadremotetimetablechunk(chunk_data: Vec<u8>, chunk_index: u32, total_chunks: u32) {
    outputlog(&format!("Received time table chunk {}/{} ({} bytes)", chunk_index + 1, total_chunks, chunk_data.len()));
    
    let mut chunks = _timetable_chunks.lock().unwrap();
    let mut total_chunks_ref = _timetable_total_chunks.lock().unwrap();
    ...
    let all_received = chunks.iter().all(|chunk| !chunk.is_empty());
    if all_received {
      outputlog("All time table chunks received, reassembling...");
      ...
      outputlog(&format!("Reassembled time table data: {} bytes", reassembled_data.len()));
      
      // Process the reassembled data using the original function logic
      Self::loadremotetimetable(reassembled_data);
  ...
  fn loadremotesignalschunk(chunk_data: Vec<u8>, chunk_index: u32, total_chunks: u32) {
    outputlog(&format!("Received signals chunk {}/{} ({} bytes)", chunk_index + 1, total_chunks, chunk_data.len()));
    
    let mut chunks = _signals_chunks.lock().unwrap();
    let mut total_chunks_ref = _signals_total_chunks.lock().unwrap();
    ...
    let all_received = chunks.iter().all(|chunk| !chunk.is_empty());
    if all_received {
      outputlog("All signals chunks received, reassembling...");
      ...
      outputlog(&format!("Reassembled signals data: {} bytes", reassembled_data.len()));
      
      // Process the reassembled data using the original function logic
      Self::loadremotesignals(reassembled_data);
